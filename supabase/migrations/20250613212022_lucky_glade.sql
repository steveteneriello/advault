/*
  ...
  1. New Features
    - Add `is_active` boolean column to campaigns table
    - Create new `campaign_manager_negative_keywords` table
    - Add sample negative keywords for existing campaigns
    - Create function to count keywords and negative keywords

  2. Security
    - Enable RLS on negative keywords table
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Allow authenticated users to read campaign_manager_negative_key'
  ) THEN
    CREATE POLICY "Allow authenticated users to read campaign_manager_negative_key"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'campaign_manager_negative_keywords' 
    AND policyname = 'Allow service role full access to campaign_manager_negative_key'
  ) THEN
    CREATE POLICY "Allow service role full access to campaign_manager_negative_key"
...
BEGIN
  ...
  FOR campaign_rec IN SELECT id FROM campaign_manager_campaigns LOOP
    ...
    FOR i IN 1..3 + floor(random() * 3)::int LOOP
      ...
      selected_keyword := neg_keywords[1 + floor(random() * array_length(neg_keywords, 1))::int];
      selected_match_type := match_types[1 + floor(random() * array_length(match_types, 1))::int];
      
      -- Insert if not exists - fixed the ambiguous column reference
      IF NOT EXISTS (
        SELECT 1 FROM campaign_manager_negative_keywords 
        WHERE campaign_id = campaign_rec.id AND keyword = selected_keyword
      ) THEN
        INSERT INTO campaign_manager_negative_keywords (campaign_id, keyword, match_type)
        VALUES (campaign_rec.id, selected_keyword, selected_match_type);
      END IF;
    END LOOP;
  END LOOP;
END $$;
