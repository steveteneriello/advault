/*
  ...
  1. New Tables
    ...
    - `campaign_manager_keywords` - Keywords for campaigns
    - `campaign_manager_keyword_stats` - Statistics for keywords
  ...
  3. Sample Data
    - Add sample plumbing category
    - Add sample drain cleaning campaign
    - Add sample keywords and statistics
*/
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM campaign_manager_campaigns WHERE name = 'Drain Cleaning Campaign') THEN
    ...
    RETURNING id INTO drain_campaign_id;
  ELSE
    SELECT id INTO drain_campaign_id FROM campaign_manager_campaigns WHERE name = 'Drain Cleaning Campaign';
  END IF;
  
  -- Insert keywords for the drain cleaning campaign if they don't exist
  IF NOT EXISTS (SELECT 1 FROM campaign_manager_keywords WHERE campaign_id = drain_campaign_id AND keyword = 'drain cleaning') THEN
    INSERT INTO campaign_manager_keywords (campaign_id, keyword, match_type)
    VALUES (drain_campaign_id, 'drain cleaning', 'broad');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM campaign_manager_keywords WHERE campaign_id = drain_campaign_id AND keyword = 'drain cleaning near me') THEN
    INSERT INTO campaign_manager_keywords (campaign_id, keyword, match_type)
    VALUES (drain_campaign_id, 'drain cleaning near me', 'phrase');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM campaign_manager_keywords WHERE campaign_id = drain_campaign_id AND keyword = 'emergency drain cleaning') THEN
    INSERT INTO campaign_manager_keywords (campaign_id, keyword, match_type)
    VALUES (drain_campaign_id, 'emergency drain cleaning', 'exact');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM campaign_manager_keywords WHERE campaign_id = drain_campaign_id AND keyword = 'roto rooter') THEN
    INSERT INTO campaign_manager_keywords (campaign_id, keyword, match_type)
    VALUES (drain_campaign_id, 'roto rooter', 'broad');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM campaign_manager_keywords WHERE campaign_id = drain_campaign_id AND keyword = 'roto rooter near me') THEN
    INSERT INTO campaign_manager_keywords (campaign_id, keyword, match_type)
    VALUES (drain_campaign_id, 'roto rooter near me', 'phrase');
  END IF;
END $$;
...
BEGIN
  FOR keyword_rec IN 
    ...
    FROM campaign_manager_keywords k
    JOIN campaign_manager_campaigns c ON k.campaign_id = c.id
    WHERE c.name = 'Drain Cleaning Campaign'
  LOOP
...
BEGIN
  ...
  -- Array of negative keywords to insert with their match types
  FOR neg_keyword, match_type IN 
    VALUES 
      ('draino', 'exact'),
      ('how to unclog drain', 'broad'),
      ('best drain cleaning treatments', 'phrase'),
      ('best drain cleaners', 'phrase'),
      ('floor drain cleaning', 'broad'),
      ('gutter drain cleaning', 'broad'),
      ('ac drain cleaning', 'broad')
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM campaign_manager_negative_keywords 
      WHERE campaign_id = drain_campaign_id AND keyword = neg_keyword
    ) THEN
      INSERT INTO campaign_manager_negative_keywords (campaign_id, keyword, match_type)
      VALUES (drain_campaign_id, neg_keyword, match_type);
    END IF;
  END LOOP;
END $$;