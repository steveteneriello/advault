...
async function processAllFiles() {
  ...
  try {
    ...
    if (!tableReady) {
      ...
      return {
        success: false,
        error: 'Failed to create/verify file_backups table'
      };
    }
    ...
    return {
      ...
      archivedCount: totalSuccess,
      failedCount: totalErrors,
      movedToFailuresCount: totalErrors
    };
  } catch (err) {
...
async function cleanupOldFiles(maxAgeDays = 7) {
  console.log(`\n🧹 Cleaning up files older than ${maxAgeDays} days...`);
  ...
  try {
    ...
    for (const dir of DIRS_TO_PROCESS) {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (fileAge > maxAgeDays) {
            // Check if file is archived before deleting
            const { data: existing } = await supabase
              .from('file_backups')
              .select('id')
              .eq('file_path', filePath)
            ...
            if (existing) {
              console.log(`File ${file} is archived (ID: ${existing.id}), deleting...`);
              fs.unlinkSync(filePath);
              console.log(`Deleted: ${file}`);
              dirDeleted++;
              totalDeleted++;
            } else {
              console.log(`File ${file} is not archived, skipping deletion`);
            }
          }
        } catch (err) {
          console.error(`Error processing file ${file}:`, err);
      ...
      console.log(`Deleted ${dirDeleted} files from ${dir}`);
    }
    
    console.log(`\n✅ Cleanup complete. Deleted ${totalDeleted} old files`);
...
async function main() {
  ...
  try {
    ...
    if (!cleanupResult.success) {
      console.error('❌ File cleanup failed:', cleanupResult.error);
    }
    
    console.log('\n✅ File archival process completed successfully');
  } catch (err) {
    console.error('❌ Fatal error:', err);
    process.exit(1);
  }
