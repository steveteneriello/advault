// SCRAPI/z-scrapi-scheduler/scheduler-web-simple.js
// A simplified web interface for SCRAPI Scheduler
...
// Create a simple mock for the tracker until we have the full system
const mockTracker = {
...
// Create a simple mock for the scheduler system
const scrapiScheduler = {
  scheduler: new OxylabsScheduler(),
  tracker: mockTracker,
  ...
  async getSchedulesSummary() {
    try {
      ...
      const schedulesResult = await this.scheduler.getAllSchedules();
      
      if (!schedulesResult.success) {
        return schedulesResult;
      }
      
      const scheduleIds = schedulesResult.schedules || [];
      const schedules = [];
      
      // Get details for each schedule
      for (const id of scheduleIds) {
        const infoResult = await this.scheduler.getScheduleInfo(id);
        
        if (infoResult.success) {
          schedules.push({
            id,
            oxylabs: infoResult.details,
            tracking: { itemCount: infoResult.details.items_count },
            active: infoResult.details.active,
            nextRunAt: infoResult.details.next_run_at
          });
        }
      }
      ...
      console.error('Error getting schedules summary:', error);
      return {
        success: false,
        error: error.message
      };
...
app.get('/', async (req, res) => {
  try {
    ...
    const summaryResult = await scrapiScheduler.getSchedulesSummary();
    const schedules = summaryResult.success ? summaryResult.schedules : [];
    
    // Calculate stats
    const activeSchedulesCount = schedules.filter(s => s.active).length;
    const totalJobsCount = 0; // We don't have job tracking yet in simplified version
    const processedJobsCount = 0;
    const successRate = 0;
    
    // Prepare schedule rows
    let scheduleRows = '';
    
    // Take only the latest 5 schedules
    ...
    recentSchedules.forEach(schedule => {
      ...
      scheduleRows += `
        <tr>
          <td>${schedule.id}</td>
          <td>${statusBadge}</td>
          <td>${schedule.oxylabs?.items_count || schedule.tracking.itemCount || 0}</td>
          <td>${schedule.nextRunAt || 'N/A'}</td>
          <td>
            <a href="/schedules/${schedule.id}" class="btn btn-primary">View</a>
          </td>
        </tr>
      `;
    });
    ...
    <div class="dashboard">
      <div class="grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-value">${processedJobsCount}</div>
        </div>
        <div class="stat-card">
          <h3>Success Rate</h3>
      ...
      <div class="card">
        ...
        <div class="actions">
          ...
          <a href="/sync" class="btn btn-secondary">Sync with Oxylabs</a>
        </div>
      </div>
    </div>
    `;
    ...
    schedules.forEach(schedule => {
      ...
      scheduleRows += `
        <tr>
          ...
          <td>${statusBadge}</td>
          <td>${schedule.oxylabs?.items_count || schedule.tracking.itemCount || 0}</td>
          <td>${schedule.oxylabs?.cron || 'N/A'}</td>
          <td>${schedule.nextRunAt || 'N/A'}</td>
          <td>${schedule.oxylabs?.end_time || 'N/A'}</td>
          <td>
            <a href="/schedules/${schedule.id}" class="btn btn-primary">View</a>
            <button class="btn ${schedule.active ? 'btn-danger' : 'btn-success'}" 
              onclick="toggleScheduleState('${schedule.id}', ${!schedule.active})">
...
app.get('/schedules/:id', async (req, res) => {
  try {
    ...
    const infoResult = await scrapiScheduler.scheduler.getScheduleInfo(scheduleId);
    ...
    const runsResult = await scrapiScheduler.scheduler.getScheduleRuns(scheduleId);
    ...
    if (runs.length > 0) {
      ...
      runs.forEach(run => {
        runsTable += `
          <tr>
            <td>${run.run_id}</td>
            <td>${run.jobs.length}</td>
            <td>${Math.round(run.success_rate * 100)}%</td>
          </tr>
        `;
      });
...
app.post('/api/schedules/:id/state', async (req, res) => {
  try {
    ...
    // Set schedule state
    const result = await scrapiScheduler.scheduler.setScheduleState(scheduleId, active);
